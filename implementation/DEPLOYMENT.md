...
**2026-09-19の追加候補**：腕試しの正答率フィードバック・候補不足時の易しい側優先と、難易度／回答テンポの任意アンケートを実装。7グループの分野別自己評価を結果ページの隣接位置に配置し、ローカル試用にも接続した。単体・DB254件、PC・スマホ52件、lint・型検査・本番ビルドが成功。[仕様と検証](TRIAL-FEEDBACK-AND-SURVEY-20260919.md)。本番DB・正式問題バンク・本番割当は未変更。Force Promoteの単独Lint省略の例外は未承認であり、別経路で回避しない。以下は過去の反映記録。
