...
**追加修正の現在地（2026-09-17）**：科学表記の下付き・上付き、回答通信の短縮、解説中の次問先読み、新版問題の編集監査を `aeda204` に実装し、指定GitHubへpush済み。234件の単体・DB検査、問題準備8件、PC・スマホ38件、lint・型検査・ビルドが通過。Vercel候補 `FT5igr4jWwF3TuYoPQ1AxdzS2x7E` は01:41:52 JST Readyだが、単独Lintの省略を伴うForce Promoteが自動承認レビューに拒否され、具体的な許可を本人へ確認中。本番は `aa84222` のまま。計測用3回答は除去済み。腕試しの難度方針は別途本人へ確認中で、採点・出題・既存問題本文は変更していない。[旧改善の復元一覧と提案](RESTORE-READABILITY-AND-SPEED-20260917.md)／[候補・計測記録](checks/readability-speed-production-20260917.json)。
