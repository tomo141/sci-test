...
観測・記録日：2026-09-16。改装プロダクトの集計・品質確認・MyASP同期のための設定。Codexの定期タスクではない。Vault保存、pg_cron・pg_net、2ジョブの作成を完了。認証付き集計のHTTP 200・アプリ完了を照合後、毎時17分の集計だけを有効化・読み戻した。スケジュールによる初回実行は未観測。MyASPの毎分同期は実同期の失敗原因を調査中で停止している。

有効化は[専用SQL](activate-hourly-20260916.sql)で、保存済みコマンド・間隔・公開設定・HTTP検証番号6・アプリの完了・検証受験の除去を確認して実行。監査記録を追加し、MyASPジョブと案内送信設定を変更していない。適用SHA-256は `34ff5ab25834e51da8376049164475548f3278a8878dbda312d1fb8367ba8987`。二重実行は拒否される。初回のsetup SQLを再実行しない。
