...
do $$
declare test_id uuid:='b570685f-e06a-47e2-a426-c17eefbfc164'; a science_attempts; v science_visitors;
begin
  select * into a from science_attempts where id=test_id for update;
  if not found or a.user_id is not null or a.kind<>'trial' or a.total<>20 or a.ordinal<>2 or a.state<>'active' or a.started_at<'2026-09-15 17:48:54+00' then raise exception 'test_attempt_guard_failed'; end if;
  ...
  insert into science_audit(action,target,reason,detail) values('deployment_smoke_cleaned',test_id::text,'Removed anonymous deployment test after immediate feedback, reload recovery and numeric-key behavior were verified; no account or scheduler processed it',jsonb_build_object('answers',2,'humanAttempt',false));
end $$;
commit;
select count(*) as remaining_test_attempts from science_attempts where id='b570685f-e06a-47e2-a426-c17eefbfc164';
