...
TurnstileとSupabaseの環境変数が未設定でも、ローカルUIと主要画面は確認できます。設定時だけ認証・不正投稿対策を有効化する想定です。
...
Vercelに通常のNext.jsアプリとして接続し、環境変数を設定してください。CloudflareはDNSを `sci-test.rikei-talk.com` に向け、TurnstileのサイトキーとシークレットをVercel環境変数に設定します。
