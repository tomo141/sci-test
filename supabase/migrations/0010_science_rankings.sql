...
returns table(place bigint,nickname text,profile_id uuid,share_id uuid,score numeric,exam_length int)
language sql stable set search_path=public as $$
  with eligible as (
    ...
    select p.nickname,case when p.is_public then p.public_id else null end as profile_id,s.id as share_id,
      case when p_kind='weekly' then (a.result->>'correctCount')::numeric else (a.result->>'total')::numeric end as score,a.total
    ...
    where a.attempt_order=1
  )
  select rank() over(order by visible.score desc) as place,visible.nickname,visible.profile_id,visible.share_id,visible.score,visible.total
  from visible where visible.score is not null order by visible.score desc,visible.nickname,visible.profile_id limit 100;
$$;
