...
export function shareCopy(result:AttemptResult,nickname:string){
  ...
  if(d.domain){const score=result.domains[d.domain].score;return {metric:score===null?`${d.domain}：測定対象の回答なし`:`${d.domain} ${score}/${scoreCeiling(result.version,true)}点（参考スコア）`,subtitle:"気になる分野を、もう20問",text:`${nickname}が${d.domain}に挑戦。あなたの得意分野も測ってみよう！`};}
  return {metric:result.total===null?"総合：測定対象の回答が不足しています":`総合 ${result.total}/${scoreCeiling(result.version)}点（参考スコア）`,subtitle:"10分野の科学マップ",text:`${nickname}の科学マップ。あなたの科学は、どこまで広い？`};
}
