...
export async function adminData(ctx:Context):Promise<AdminData>{
  ...
  const [drafts,feedback,jobs,metrics,experiment,corrections,quality,holds]=await Promise.all([
  ...
  return {observedAt:new Date().toISOString(),submissions:pending,feedback:reports,quality:qualitySignals,
