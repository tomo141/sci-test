...
export async function rankings(kind:"weekly"|"full",period:"day"|"week"|"month",length:50|100=50){
  ...
  const rows=checked(await db.rpc("science_rankings",{p_kind:kind,p_start:bounds.start,p_end:bounds.end,p_length:kind==="weekly"?10:length,p_model:MODEL_VERSION,p_week:kind==="weekly"?bounds.key:null})) as RankingRow[];
