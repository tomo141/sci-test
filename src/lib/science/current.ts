...
export async function currentEstimate(db:Context["db"],owner:{userId:string}|{visitorId:string}){
  ...
  const fields=await Promise.all(domains.map(async(domain)=>{
    let query=db.from("science_responses").select("domain,a,b,c,is_correct,answered_at").eq("domain",domain).eq("eligible",true).in("model_version",evidenceModels(version));
  ...
  const result=scoreResponses(fields.flat(),true,version);
