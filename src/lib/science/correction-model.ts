...
export function correctedResult(attempt:Attempt,issued:Issued[],answers:Answer[],state:{epoch:number;updates:Map<string,RevisionUpdate>}):AttemptResult{
  ...
  const responses=answers.map(answer=>{
    ...
    return {...q.snapshot,correct:answer.is_correct,eligible:q.eligible&&!excluded.has(q.ordinal),answeredAt:answer.answered_at};
  });
  ...
  return {...scoreResponses(responses),definition:attempt.definition,originalAnswerCount:attempt.total,
    ...
    correctionEpoch:state.epoch,corrections:{count:updates.length,excludedCount:excluded.size,updatedAt:updates.map(u=>u.approved_at).sort().at(-1)??null}};
}
