...
export async function correctionState(db:Context["db"],revisions:string[]){
  ...
  const rows=revisions.length?checked(await db.from("science_revision_updates").select("*").in("source_revision",[...new Set(revisions)])) as RevisionUpdate[]:[];
  return {epoch,updates:new Map(rows.map(row=>[row.source_revision,row]))};
}
export async function refreshCorrectedAttempt(db:Context["db"],attempt:Attempt):Promise<Attempt>{
  ...
  const state=await correctionState(db,[]);
