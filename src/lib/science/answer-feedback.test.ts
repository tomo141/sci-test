...
const issued = {
  ...
  snapshot: { content: { question: "1 + 1 は？", choices: ["1", "2", "3", "4"], correctIndex: 1,
...
describe("answer feedback", () => {
  it("uses the displayed choice order for grading and the canonical order for answer text", () => {
    expect(answerExplanation(issued, 3)).toMatchObject({ ordinal: 19, correct: true, selectedAnswer: "2", correctAnswer: "2" });
  ...
  it("shows an approved correction without falsely grading a withdrawn item", () => {
    ...
    expect(answerExplanation(issued, 3, update)).toMatchObject({ correct: null, correctAnswer: "3", selectedAnswer: "2", correctionNote: update.reason });
  });
