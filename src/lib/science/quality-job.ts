...
export async function runQualityWatch(db: ReturnType<typeof service>, jobId: string) {
  ...
  const counts = await Promise.all([
    db.from("science_quality_responses").select("revision_id", { count: "exact", head: true }).eq("quality_eligible", true).eq("model_version", MODEL_VERSION).gte("answered_at", from).lt("answered_at", to),
  ...
  const [items, raw, reports] = await Promise.all([
    ...
    readAll<ResponseRow>((a, b) => db.from("science_quality_responses").select("user_id,visitor_id,family_id,revision_id,domain,a,b,c,is_correct,answered_at,canonical_choice,eligible").eq("quality_eligible", true).eq("model_version", MODEL_VERSION).gte("answered_at", from).lt("answered_at", to).order("answered_at").order("attempt_id").order("ordinal").range(a, b)),
  ...
  const result = qualitySignals(items, answers, feedback, now);
