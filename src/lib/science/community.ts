...
import { levelReferenceInput, type LevelReference } from "./knowledge-levels";
...
export const draftInput=z.object({id:z.string().uuid(),revision:z.number().int().min(0),domain:z.enum(domains),subdomain:z.string().trim().min(1).max(100),content:draftContent,creditName:z.string().trim().max(30),aiAssisted:z.boolean(),levelReference:levelReferenceInput.nullable().optional()}).strict().refine(d=>isSubdomainOf(d.domain,d.subdomain),"分野を選び直してください");
export type Draft={id:string;revision:number;domain:ScienceDomain;subdomain:string;content:Content;state:string;review_note:string|null;revision_id:string|null;updated_at:string;author_credit:string;ai_assisted:boolean;level_reference?:LevelReference|null};
...
export async function communityData(ctx:Context){
  ...
  const [drafts,inputs]=await Promise.all([
    ctx.db.from("science_submission_drafts").select("id,revision,domain,subdomain,content,state,review_note,revision_id,updated_at,author_credit,ai_assisted,level_reference").eq("author_id",ctx.userId).order("updated_at",{ascending:false}).limit(100),
...
export async function saveDraft(ctx:Context,input:z.infer<typeof draftInput>){
  ...
  const existing=await ctx.db.from("science_submission_drafts").select("id,revision,state,domain,subdomain").eq("id",input.id).eq("author_id",userId).maybeSingle();
  if(existing.error)checked(existing);
  const changedScope=existing.data&&(existing.data.domain!==input.domain||existing.data.subdomain!==input.subdomain);
  const fields={domain:input.domain,subdomain:input.subdomain,content:input.content,updated_at:new Date().toISOString(),author_credit:input.creditName,ai_assisted:input.aiAssisted,
    ...(input.levelReference!==undefined?{level_reference:input.levelReference}:changedScope?{level_reference:null}:{})};
  if(!existing.data){
