...
export const questionContent = z.object({
  ...
  sources:z.array(z.object({title:z.string().trim().min(2).max(300),url:z.string().url().refine(url=>/^https?:\/\//.test(url),"HTTP(S)の出典URLを入力してください")}).strict()).min(1).max(5)
...
export const draftInput=z.object({id:z.string().uuid(),revision:z.number().int().min(0),domain:z.enum(domains),subdomain:z.string().trim().min(1).max(100),content:draftContent}).strict().refine(d=>isSubdomainOf(d.domain,d.subdomain),"分野を選び直してください");
export type Draft={id:string;revision:number;domain:ScienceDomain;subdomain:string;content:Content;state:string;review_note:string|null;revision_id:string|null;updated_at:string};
...
export async function communityData(ctx:Context){
  ...
  const [drafts,inputs]=await Promise.all([
    ctx.db.from("science_submission_drafts").select("id,revision,domain,subdomain,content,state,review_note,revision_id,updated_at").eq("author_id",ctx.userId).order("updated_at",{ascending:false}).limit(100),
...
export async function saveDraft(ctx:Context,input:z.infer<typeof draftInput>){
  ...
  const fields={domain:input.domain,subdomain:input.subdomain,content:input.content,updated_at:new Date().toISOString()};
  if(!existing.data){
