...
function ReviewCard({card,reload}:{card:Card;reload:()=>Promise<void>}){
  ...
  return <AppCard><p className="text-xs font-bold">{card.domain}</p><h2 className="mt-3 whitespace-pre-wrap text-xl font-bold leading-8"><ScienceText>{card.content.question}</ScienceText></h2><fieldset disabled={revealed} className="mt-5 grid gap-3"><legend className="sr-only">答えを選ぶ</legend>{card.content.choices.map((c,i)=><label key={i} className="flex gap-3 rounded-xl border p-3"><input type="radio" name={card.revision_id} checked={answer===i} onChange={()=>setAnswer(i)}/><span><ScienceText>{c}</ScienceText></span></label>)}</fieldset>
    {card.correction&&<p className="mt-4 rounded-xl bg-amber-50 p-4 text-sm whitespace-pre-wrap">訂正後の問題で復習しています。{card.correction}</p>}
    {!revealed?<AppButton className="mt-5" variant="secondary" onClick={()=>setRevealed(true)}>解説を確認する</AppButton>:<><p className="mt-5 font-bold">{answer===null?"正解を確認":answer===card.content.correctIndex?"正解！":"もう一度、覚えておこう"}：<ScienceText>{card.content.choices[card.content.correctIndex]}</ScienceText></p><p className="mt-4 whitespace-pre-wrap leading-8"><ScienceText>{card.content.explanation}</ScienceText></p><div className="mt-3 text-sm">{card.content.sources.map((s,i)=><p key={i}>{s.url&&/^https?:\/\//.test(s.url)?<a href={s.url} target="_blank" rel="noreferrer" className="underline">{s.title}</a>:s.title}</p>)}</div><div className="mt-5 flex flex-wrap gap-3"><AppButton disabled={busy} onClick={()=>void mark(true)}>{operation?"保存を再確認する":"理解できた"}</AppButton>{!operation&&<AppButton disabled={busy} variant="secondary" onClick={()=>void mark(false)}>明日もう一度</AppButton>}</div></>}
