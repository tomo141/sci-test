  ...
  return <main className="page-container max-w-4xl py-8">
    ...
    <AppCard className="mt-6"><h2 className="text-2xl font-black">解説・出典を読む</h2><p className="mt-3">間違えた問題や気になる問題を、少しずつ復習できます。</p><AppButton variant="secondary" className="mt-4" onClick={async()=>{try{setReview(await scienceApi<Review>("review",{attemptId}));setError("");}catch(e){setError((e as Error).message);}}}>解説を開く</AppButton>
      {review&&review.rows.slice(reviewPage*10,reviewPage*10+10).map((row)=><section key={row.ordinal} className="mt-6 border-t pt-5"><p className="text-sm font-bold">第{row.ordinal+1}問 · {row.domain} · {row.selectedIndex===null||row.update?.excluded?"採点対象外":row.correct?"正解":"不正解"}</p>
        ...
        {row.selectedIndex===null?<p className="mt-3 leading-8">問題の取り下げにより、未回答でスキップしました。正誤や回答数には含めません。</p>:row.content&&<><h3 className="mt-2 whitespace-pre-wrap font-bold leading-8"><ScienceText>{row.content.question}</ScienceText></h3><p className="mt-3">あなたの回答：<ScienceText>{row.content.choices[row.selectedIndex]}</ScienceText></p><p className="mt-2 font-bold">{row.update?"受験時の正解キー":"正解"}：<ScienceText>{row.content.choices[row.content.correctIndex]}</ScienceText></p><p className="mt-3 whitespace-pre-wrap leading-8"><ScienceText>{row.content.explanation}</ScienceText></p></>}
        {row.creditName&&<p className="mt-3 text-xs">作問：{row.creditName}</p>}
        {row.update&&<div className="mt-5 rounded-xl border p-4"><h4 className="font-bold">訂正後の問題・解説</h4><p className="mt-3 whitespace-pre-wrap leading-8"><ScienceText>{row.update.content.question}</ScienceText></p><ol className="mt-3 list-decimal pl-6">{row.update.content.choices.map((c,i)=><li key={i}><ScienceText>{c}</ScienceText>{i===row.update!.content.correctIndex&&"（正解）"}</li>)}</ol><p className="mt-3 whitespace-pre-wrap leading-8"><ScienceText>{row.update.content.explanation}</ScienceText></p>{row.update.content.sources.map((s,i)=><p key={i} className="mt-2 text-sm">{s.url&&/^https?:\/\//.test(s.url)?<a href={s.url} rel="noreferrer" target="_blank" className="underline">{s.title}</a>:s.title}</p>)}</div>}
        {row.content&&<ul className="mt-3 list-inside list-disc text-sm">{row.content.sources.map((s,i)=><li key={i}>{s.url&&/^https?:\/\//.test(s.url)?<a href={s.url} target="_blank" rel="noreferrer" className="underline">{s.title}</a>:s.title}</li>)}</ul>}
        {row.selectedIndex!==null&&<div className="mt-3 flex flex-wrap gap-2"><AppButton variant="ghost" onClick={async()=>{try{await scienceApi("bookmark",{attemptId,revisionId:row.revisionId,enabled:true});setNote("復習用に保存しました。");}catch(e){const issue=e as RequestError;setError(issue.code==="registration_required"?"ブックマークは無料登録後に利用できます。":issue.message);}}}>復習用に保存</AppButton><AppButton variant="ghost" onClick={()=>setReportOrdinal(row.ordinal)}>問題の改善を提案</AppButton></div>}
