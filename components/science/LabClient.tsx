...
import { newLevelReference } from "@/src/lib/science/knowledge-levels";
...
export function LabClient(){
  ...
  async function save(submit=false){
    ...
    try{
      const saved=await scienceApi<{id:string;revision:number}>("save",{id:draft.id,revision:draft.revision,domain:draft.domain,subdomain:draft.subdomain,content:draft.content,creditName:draft.author_credit,aiAssisted:draft.ai_assisted,levelReference:draft.level_reference??null},"science-community");
      setDraft({...draft,revision:saved.revision});
      if(submit){await scienceApi("submit",{id:saved.id,revision:saved.revision,licenseVersion:data?.licenseVersion,licenseHash:data?.licenseHash,rights,adultOrGuardianConsent:guardian},"science-community");setDraft(null);setNote("投稿しました。運営の確認後にラボへ登場します。");}
  ...
  return <main className="page-container max-w-5xl py-10">
    ...
    {data&&!data.open&&<p className="mt-6 rounded-xl border bg-white p-4 text-sm leading-7">投稿受付の開始を準備しています。登録すると下書きを保存できます。受付条件が整い次第、このページから投稿できるようになります。<Link href="/contribution-terms" className="ml-2 underline">確認中の投稿条件を見る</Link></p>}
    {draft&&<AppCard className="mt-8"><div className="flex items-center justify-between gap-4"><h2 className="text-2xl font-black">{states[draft.state]} · あなたの科学の問い</h2><button onClick={()=>setDraft(null)} className="shrink-0 underline">閉じる</button></div>
      ...
      <fieldset disabled={!editable||busy} className="mt-6 grid gap-5">
        ...
        <div className="grid gap-5 sm:grid-cols-2"><label className="font-bold">大分野<select value={draft.domain} className={inputClass} onChange={e=>{const domain=e.target.value as ScienceDomain;setDraft({...draft,domain,subdomain:subdomainsByDomain[domain][0],level_reference:null});}}>{domains.map(d=><option key={d}>{d}</option>)}</select></label><label className="font-bold">小分野<select value={draft.subdomain} className={inputClass} onChange={e=>setDraft({...draft,subdomain:e.target.value,level_reference:null})}>{subdomainsByDomain[draft.domain].map(d=><option key={d}>{d}</option>)}</select></label></div>
        <details className="rounded-xl border p-4"><summary className="cursor-pointer font-bold">どの段階の人に解けそう？（作問の目安）</summary><div className="mt-4"><KnowledgeLevelRadios scope={{domain:draft.domain,subdomain:draft.subdomain}} name="author-level" value={draft.level_reference?.level} onChange={level=>setDraft({...draft,level_reference:{...(draft.level_reference??newLevelReference()),level}})} purpose="author"/><fieldset className="mt-5"><legend className="font-bold">見積もりへの自信</legend><div className="mt-3 flex flex-wrap gap-4">{([["confident","自信がある"],["uncertain","少し迷う"],["unknown","判断できない"]] as const).map(([value,label])=><label key={value} className="flex gap-2"><input type="radio" name="author-confidence" checked={draft.level_reference?.confidence===value} onChange={()=>setDraft({...draft,level_reference:{...(draft.level_reference??newLevelReference()),confidence:value}})}/>{label}</label>)}</div></fieldset></div></details>
        ...
        <div><p className="font-bold">確認できる出典</p><p className="mt-2 text-sm leading-7">Webページの名称とURL、または書名・版・該当ページを記入してください。</p>{draft.content.sources.map((source,index)=><div key={index} className="mt-4 grid gap-5 rounded-xl border p-4 sm:grid-cols-2"><label className="font-bold">出典{index+1}の名称・書誌情報<input value={source.title} maxLength={300} className={inputClass} onChange={e=>changeContent({sources:draft.content.sources.map((s,i)=>i===index?{...s,title:e.target.value}:s)})}/></label><label className="font-bold">URL（Webの出典の場合）<input type="url" value={source.url??""} maxLength={2000} className={inputClass} onChange={e=>changeContent({sources:draft.content.sources.map((s,i)=>i===index?{...s,url:e.target.value}:s)})}/></label>{draft.content.sources.length>1&&<button type="button" className="justify-self-start text-sm underline" onClick={()=>changeContent({sources:draft.content.sources.filter((_,i)=>i!==index)})}>この出典を削除</button>}</div>)}{draft.content.sources.length<5&&<button type="button" className="mt-4 text-sm underline" onClick={()=>changeContent({sources:[...draft.content.sources,{title:"",url:""}]})}>出典を追加する</button>}</div>
        <EditorialHints content={draft.content} />
      </fieldset>
      {editable&&<><AppButton className="mt-6" variant="secondary" disabled={busy} onClick={()=>void save()}>下書きを保存</AppButton>
        {data?.open&&data.licenseVersion&&<div className="mt-6 rounded-xl bg-[var(--color-page)] p-5"><p className="text-sm leading-7">{data.licenseSummary}</p><Link href={`/contribution-terms?version=${encodeURIComponent(data.licenseVersion)}`} target="_blank" className="mt-3 inline-block text-sm underline">投稿規約の全文を読む（{data.licenseVersion}）</Link><label className="mt-4 flex gap-3"><input type="checkbox" checked={rights} onChange={e=>setRights(e.target.checked)}/><span>投稿規約を確認し、この内容を投稿・利用許諾できる権利を持っていることを確認しました。</span></label><label className="mt-4 flex gap-3"><input type="checkbox" checked={guardian} onChange={e=>setGuardian(e.target.checked)}/><span>18歳以上、または保護者の同意を得ています。</span></label><AppButton className="mt-5" disabled={busy||!rights||!guardian} onClick={()=>void save(true)}>{busy?"保存中…":"確認を依頼して投稿する"}</AppButton></div>}
      </>}
    </AppCard>}
    {data?.signedIn&&<><h2 className="mt-12 text-2xl font-black">あなたの作問ノート</h2><div className="mt-5 grid gap-3">{data.drafts.map(d=><button key={d.id} onClick={()=>open(d)} className="rounded-xl border bg-white p-5 text-left hover:border-[var(--color-primary-700)]"><span className="text-xs font-bold text-[var(--color-primary-700)]">{states[d.state]} · {d.domain}</span><p className="mt-2 font-bold"><ScienceText>{d.content.question||"まだ題名のない問い"}</ScienceText></p></button>)}{!data.drafts.length&&<p className="text-[var(--color-muted)]">あなたの最初の問いをお待ちしています。</p>}</div>
    ...
    <AppCard className="mt-12"><h2 className="text-xl font-black">一緒に育てる、科学の問題バンク</h2><p className="mt-4 leading-8">投稿 → 出典・権利・正解の確認 → ラボで出題 → 回答と改善報告の検証 → 正式問題へ。ラボで解いた問題の成績は、実力スコアと本試験ランキングには加えません。</p><p className="mt-3 text-sm leading-7">良質な改善報告、ラボの参加、採用された作問を記録します。理系とーくバーで「解こう＆作問しようBar」などの企画も予定しています。開催日は決まり次第ご案内します。</p></AppCard>
