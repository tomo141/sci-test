...
export function CorrectionEditor({revisionId,original,reload}:{revisionId:string;original:Content;reload:()=>Promise<void>}){
  const [open,setOpen]=useState(false),[content,setContent]=useState<Content>(structuredClone(original)),[mode,setMode]=useState("exclude"),[reason,setReason]=useState(""),[busy,setBusy]=useState(false),[error,setError]=useState("");
  if(!open)return <AppButton className="mt-4" variant="secondary" onClick={()=>setOpen(true)}>この問題の修正版を用意する</AppButton>;
  return <div className="mt-6 border-t pt-5"><h3 className="text-xl font-bold">修正版をつくる</h3><p className="mt-3 text-sm leading-7">元の問題・回答は保存します。別の管理者が確認すると、新しい出題から旧問題を外し、影響する成績を訂正します。新版は次の問題バンクへ加える候補になります。</p>
    <fieldset disabled={busy} className="mt-4 grid gap-4"><label>訂正の範囲<select value={mode} className={field} onChange={e=>setMode(e.target.value)}><option value="exclude">正誤に影響する訂正（旧問題は全員の採点から除外）</option><option value="explanation">解説・出典だけの訂正（点数は維持）</option></select></label>
