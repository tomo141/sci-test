#!/usr/bin/env node
/**
 * Merge batch50 JSON files → knowledgeQuestionsBatch50.ts + questionLevels.json entries
 */
...
const OUT_TS = join(ROOT, "src/lib/data/knowledgeQuestionsBatch50.ts");
...
function toBankQuestion(raw, seq) {
  ...
  if (!tags.includes("batch50")) tags.push("batch50");
...
for (const file of DOMAIN_FILES) {
  ...
  if (items.length !== 50) {
    console.error(`${file}: expected 50 questions, got ${items.length}`);
    process.exit(1);
  }
  rawItems.push(...items);
}
...
if (rawItems.length !== 500) {
  console.error(`Expected 500 questions, got ${rawItems.length}`);
  process.exit(1);
}
