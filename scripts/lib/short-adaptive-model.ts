...
import { readingLoad, TRIAL_POLICY, LEGACY_TRIAL_POLICY, type TrialPolicy } from "../../src/lib/science/trial-policy";
...
export function choose(bank: Draft[], answers: SavedAnswer[], history: SavedAnswer[], random = Math.random, policy: TrialPolicy = TRIAL_POLICY) {
  ...
  const selected = selectCandidate(candidates, responses(bank,answers), { ...definition("trial"), selectionPolicy: policy }, random, responses(bank,history));
  return selected ? { id:selected.candidate.revisionId, predicted:selected.predicted, reason:selected.reason, probability:selected.selectionProbability, target:selected.target } : null;
}
...
export { experienceQuestions, experienceLabel } from "../../src/lib/science/experience-survey";
export { knowledgeLevels, scopeLabel, KNOWLEDGE_LEVEL_VERSION } from "../../src/lib/science/knowledge-levels";
export { TRIAL_POLICY, LEGACY_TRIAL_POLICY };
