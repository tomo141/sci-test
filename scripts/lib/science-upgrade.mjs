...
export async function buildUpgrade(stage){
  ...
  const selected=stage==='followups'?later:names.filter(n=>n.startsWith('0007_'));
  const required=stage==='followups'?base:[...base,...later];
  ...
  const through=later.at(-1).slice(0,4),name=stage==='followups'?`followup-schema-0020-${through}.sql`:`security-cutover-after-${through}.sql`;
