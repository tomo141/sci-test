...
export default async function ContributionTerms({searchParams}:{searchParams:Promise<{version?:string}>}){
  const query=await searchParams;
  let version=query.version;
  if(version&&!/^[a-zA-Z0-9._-]{1,100}$/.test(version))notFound();
  if(!version){const config=await releaseConfig();if(config.labSubmissions)version=config.licenseVersion??undefined;}
  const license=version?await publishedLicense(service(),version):null;
  ...
  return <><SiteHeaderWithAuth/><main className="page-container max-w-4xl py-10"><AppCard><Link href="/lab" className="text-sm underline">みんなの出題ラボへ</Link><h1 className="mt-5 text-3xl font-black">{license?"投稿規約":"投稿条件（確認中の案）"}</h1>
    {license?<><p className="mt-4 text-sm leading-7">運営者：{license.operator_name}<br/>版：{license.version} · 公開日：{new Date(license.published_at).toLocaleDateString("ja-JP",{timeZone:"Asia/Tokyo"})}</p><p className="mt-5 rounded-xl bg-[var(--color-page)] p-5 leading-8">{license.terms.summary}</p>{!license.active&&<p className="mt-4 text-sm">過去の投稿に適用された版です。現在の新規投稿には適用していません。</p>}</>:<><p className="mt-4 rounded-xl bg-amber-50 p-5 leading-8">投稿条件を確認しているため、現在は下書きの保存まで利用できます。以下は確認用の案で、投稿の契約条件として適用しているものではありません。</p><p className="mt-5 leading-8">著作権は作者に残し、理系とーくが問題の公開・改善・商用提供を継続できる利用許諾を得る方針です。</p></>}
